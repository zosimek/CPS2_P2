function [standard, deviant, time] = epoch_EEG(EEG, events, standard_event_mark, deviant_event_mark, baseline_time_period, epoch_time_length)
% OUTPUT:
    ...
    for event_no = 1:length(EEG.event)
        if strcmp(events(event_no).type, standard_event_mark)
            %disp('standard')
            for channel_no = 1:size(EEG.chanlocs, 2)
                ...
                if section_end <= size(EEG.filtered, 2) && section_start > 0
                    dataSegment = EEG.filtered(channel_no, section_start:section_end);
                    standard{channel_no, standard_col} = dataSegment;
                end
            end
        ...
        elseif strcmp(events(event_no).type, deviant_event_mark)
            ...
            for channel_no = 1:size(EEG.chanlocs, 2)
                ...
                if section_end <= size(EEG.filtered, 2) && section_start > 0
                    dataSegment = EEG.filtered(channel_no, section_start:section_end);
                    deviant{channel_no, deviant_col} = dataSegment;
                end
            end
